...
//Class - Static & Dynamic Arrays
class AryDyn{          //*******By Convention: Use a capital letter to start a variable name for structures*******
 public:               //normally in a class you hide the data, but to convert from a structure to a class you have to make it public
    int **data;           //*******Indicates when you see it that you need to go to where it's defined to figure out what it is?
  int rowsUtl;
  int colsUtl;
};
class AryStat{         //Static Array, needs the global const SIZE (the other is a dynamic array)
public:
    int data[ROWS][COLS];
  int rowsUtl;
  int colsUtl;
